...
int main() {
    // Example 1: Create mg_str from C string
    struct mg_str s = mg_str("Hello,World");

    printf("Original mg_str: %.*s (length %zu)\n", (int) s.len, s.buf, s.len);

    // Example 2: Split string with mg_span
    struct mg_str left, right;
    if (mg_span(s, &left, &right, ',')) {
        printf("Split left: %.*s\n", (int) left.len, left.buf);
        printf("Split right: %.*s\n", (int) right.len, right.buf);
    }

    // Example 3: Compare strings (case-sensitive and case-insensitive)
    struct mg_str str1 = mg_str("abc");
    struct mg_str str2 = mg_str("Abc");

    printf("mg_strcmp result: %d\n", mg_strcmp(str1, str2));      // Should be non-zero
    printf("mg_strcasecmp result: %d\n", mg_strcasecmp(str1, str2)); // Should be 0 (equal ignoring case)

    // Example 4: Pattern matching
    struct mg_str target = mg_str("foobar123");
    struct mg_str pattern = mg_str("foo*123");

    if (mg_match(target, pattern, NULL)) {
    ...
    struct mg_str num_str = mg_str("12345");
    uint32_t number = 0;

    if (mg_str_to_num(num_str, 10, &number, sizeof(number))) {
        printf("Converted number: %u\n", number);
    } else {
        printf("Failed to convert string to number.\n");
    }

    // Example 6: String to hex number
    struct mg_str hex_str = mg_str("0x1A3F");
    uint32_t hex_number = 0;

    if (mg_str_to_num(hex_str, 0, &hex_number, sizeof(hex_number))) {
        printf("Converted hex number: 0x%X\n", hex_number);
    } else {
